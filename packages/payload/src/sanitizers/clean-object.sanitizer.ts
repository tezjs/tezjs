...
function clear(jsonObject: { [key: string]: any }, isRoot: boolean): any {
  ...
  let ignoreNestedChildrens = commonContainer.tezConfig.payload.ignoreNestedChildrens || [];
  const version = 4;
  var jObject = {};
    ...
    if (jsonObject[t] && isObject(jsonObject[t])) {
      if (!ignoreColumns(t, isRoot)) {
        if (version === 4 && keyName === "attributes") {
          ...
          if (ignoreNestedChildrens && ignoreNestedChildrens.includes(t)) {
            jsonObject[t]?.data.forEach((element) => {
                const keys = Object.keys(element.attributes);;
                const ignoreKeys = ignoreNestedChildrens.filter((t) => keys.includes(t));
              if (
                ignoreKeys.length > 0
              )
              ignoreKeys.forEach((t) => element.attributes[t] = undefined);
            });
          }
      ...
      if (t === "LongText" || t === "Content") {
        ...
        content = content
          ? content.split("\n").map((x: any) => x.trim())
          : content;
        if (htmlSanitizer) content = htmlSanitizer(content);
