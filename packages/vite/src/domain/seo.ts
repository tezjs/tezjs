...
export class Seo extends HtmlElement {
    ...
    addPreloads(){
        if((<Head>this.seo).preloads){
            (<Head>this.seo).preloads.forEach(item=>{
                if(item.type === "module")
    ...
    addPreload(path:string,as:string,forAll:boolean=false){
        this.addElement(`<link data-head="tezjs-preload" rel="preload" as="${as}" ${as === "script" ? "crossorigin":''}  href="${forAll? path : `/${path.replace(/\/\//g, "/")}`}">`,true)
    }


    addModulePreload(path:string,forAll:boolean=false){
        this.addElement(`<link data-head="tezjs-preload"  rel="modulepreload" href="${forAll? path : `/${path}`}">`,true)
    }
